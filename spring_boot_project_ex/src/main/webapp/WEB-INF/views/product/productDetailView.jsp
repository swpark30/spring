	...
	<head>
		...
		<script>
			...
			function qtyChange(num){
				qty = qty + num;
				if(qty<1) qty = 1; // 1 미만이 안 되도록 설정
				calAmount(); // 계산하는 함수 호출
			}
		...
		<div id="wrap">
			...
			<section>
				...
				<table border="1" width="800">
					...
					<tr>
						<td>상품번호</td><td><input type="hidden" name="prdNo" value="${prd.prdNo}">${prd.prdNo}</td>
					</tr>
					<tr>
						<td>상품명</td><td>${prd.prdName}</td>
					</tr>
					<tr>
						<td>가격</td><td><fmt:formatNumber value="${prd.prdPrice}" pattern="#,###"/>원</td>
					</tr>
					<tr>
					...
					<tr>
						<td>구매 예정 금액</td>
						<td>
							<span id="amount"><fmt:formatNumber value="${prd.prdPrice}" pattern="#,###"/>원</span>
						</td>
					</tr>
					<tr>
						<td colspan="2">
